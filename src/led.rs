...
impl Actor<LedCmd, NoEvent> for Led {
    fn init(&mut self,scheduler:&mut TimerScheduler<LedCmd>) {
        info!("Led init");
        scheduler.interval_timer(
            LedCmd::TimerBlink,
    ...
    fn on(&mut self, cmd: &LedCmd,scheduler:&mut TimerScheduler<LedCmd>,publisher:&mut dyn Publisher<NoEvent>) {
        info!("Led cmd {:?}", cmd);
